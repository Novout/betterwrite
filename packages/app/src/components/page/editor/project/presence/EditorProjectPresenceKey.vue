<template>
  <div v-if="props.id" class="flex flex-col gap-2 w-full">
    ...
    <div class="flex w-full items-center rounded-full p-1 justify-between">
      <p :class="[blurLink ? 'filter blur' : '']" class="cursor-pointer underline font-bold text-lg" @click="blurLink = !blurLink">{{ pathLink }}</p>
...
const pathLink = computed(() => `${env.getCorrectLocalUrl()}/?liveshare=${props.id}`)

const onCopy = async (isLink = false) => {
  const { copy, isSupported } = useClipboard({ source: isLink ? pathLink.value : props.id })
