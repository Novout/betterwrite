  ...
  <Modal @close="onClose">
    ...
    >
      <EditorAbsoluteHeader
        :title="t('editor.schemas.create.title')"
        @close="onClose"
      />
      <p class="mb-10 font-raleway text-lg">
        {{ t('editor.schemas.create.description') }}
      </p>
      <form class="flex flex-col gap-10 w-full" @submit.prevent.stop="">
        <div class="flex flex-col gap-2">
          <label class="text-lg font-bold font-poppins">{{
            t('editor.schemas.create.name')
          }}</label>
          <InputText
            v-model="options.name"
            class="p-2 bg-theme-background-2 rounded shadow wb-text"
          />
        </div>
        <div class="flex flex-col gap-2">
          <label class="text-lg font-bold font-poppins">{{
          ...
          <div class="flex items-center gap-2">
            <InputSelect
              ...
              class="w-36"
              :arr="[
                t('editor.schemas.types.default.target'),
                t('editor.schemas.types.characters.target'),
              ]"
            />
            <p v-if="setter === 'default'">
              {{ t('editor.schemas.types.default.description') }}
            </p>
            <p v-else-if="setter === 'characters'">
              {{ t('editor.schemas.types.characters.description') }}
            </p>
          </div>
        </div>
        <div class="flex flex-col gap-2">
          <label class="text-lg font-bold font-poppins">{{
            t('editor.schemas.create.prefix')
          }}</label>
          <p>{{ t('editor.schemas.create.prefixDescription') }}</p>
          <InputChar v-model="options.prefix" />
        </div>
        <Button @click.prevent.stop="onCreate">{{
          t('editor.schemas.create.button')
        }}</Button>
      </form>
    </div>
  </Modal>
...
<script setup lang="ts">
  ...
  const setter = computed(() =>
    transformer.schemas().type(options.type, 'setter'),
  )
